...
namespace Vec64x61 {
    ...
    func add{range_check_ptr}(a: (felt, felt, felt), b: (felt, felt, felt)) -> (
        res: (felt, felt, felt)
    ) {
    ...
    func sub{range_check_ptr}(a: (felt, felt, felt), b: (felt, felt, felt)) -> (
        res: (felt, felt, felt)
    ) {
    ...
    func mul{range_check_ptr}(a: (felt, felt, felt), b: felt) -> (res: (felt, felt, felt)) {
    ...
    func dot{range_check_ptr}(a: (felt, felt, felt), b: (felt, felt, felt)) -> (res: felt) {
        let (x) = Math64x61.mul(a[0], b[0]);
        let (y) = Math64x61.mul(a[1], b[1]);
        let (z) = Math64x61.mul(a[2], b[2]);
        let (res) = Math64x61.add(x + y, z);
        return (res,);
    }

    // Calculates the cross product of two 3D vectors
    func cross{range_check_ptr}(a: (felt, felt, felt), b: (felt, felt, felt)) -> (
        res: (felt, felt, felt)
    ) {
