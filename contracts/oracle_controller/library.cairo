...
func OracleController_get_value_for_publisher{
    syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr
}(key : felt, publisher : felt) -> (value : felt, last_updated_timestamp : felt):
    alloc_locals

    let (
        primary_oracle_implementation_address
    ) = OracleController_primary_oracle_implementation_address_storage.read()
    let (value, last_updated_timestamp) = IOracleImplementation.get_value_for_publisher(
        primary_oracle_implementation_address, publisher, key
    )
    return (value, last_updated_timestamp)
end
