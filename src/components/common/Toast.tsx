      ...
      {isVisible && (
        <div className={styles.toast}>
          <div className="flex w-full flex-row items-center gap-4 rounded-t-2xl bg-lightBlur px-5	py-3 sm:gap-10 md:gap-16">
            ...
            {type === "fail" && (
              <div className="flex flex-row items-center justify-center gap-3">
                <XCircle className="h-6 w-6 text-redDown" />
            ...
            {type === "information" && (
              <div className="flex flex-row items-center justify-center gap-3">
                <Info className="h-6 w-6 text-lightGreen" />
            ...
            <span className={styles.close} onClick={handleCloseToast}>
              {type === "success" && <X className="h-5 w-5 text-mint" />}
              {type === "fail" && <X className="h-5 w-5 text-redDown" />}
              {type === "information" && (
                <X className="h-5 w-5 text-lightGreen" />
              )}
            </span>
          </div>
