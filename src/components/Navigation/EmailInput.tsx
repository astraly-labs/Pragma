...
const InputComponent = ({ placeholderText, className }: InputProps) => {
  ...
  const handleSubmit = async () => {
    try {
      const res = await fetch("https://blog.pragma.build/#portal", {
        method: "HEAD",
      });
      setIsSubmitted(true);
    } catch {
  ...
  return (
    <div>
      <div className="flex flex-row justify-between gap-3">
        ...
        >
          <ArrowRight className="w-3 cursor-pointer" />
        </button>
      </div>
      ...
      {isSubmitted && (
        <Toast
          ...
          text="You will now start receiving emails from Pragma."
        />
      )}
      <label className={clsx("flex gap-3 pt-4", styles.checkbox)}>
        <input
          type="checkbox"
